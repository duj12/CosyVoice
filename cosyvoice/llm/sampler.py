...
class Sampler(nn.Module):
    ...
    def forward(
        ...
        # Post process logits
        logits = logits.div(
            torch.FloatTensor([[temperature]]).to(logits.device))  # [1,1]
        ...
        batch_next_token_ids, success = top_k_top_p_sampling_from_probs(
            probs,
            uniform_samples,
            torch.LongTensor([top_k]).to(logits.device).repeat(batch_size),
            # [1]
            torch.FloatTensor([top_p]).to(logits.device).repeat(batch_size),
            # [1]
...
def ras_sampling2(weighted_scores_in, decoded_tokens_in, top_p=0.8, top_k=25,
    ...
    rep_top_ids = sampler(weighted_scores_in, resample['temperature'],
                          top_p=top_p + resample['topp'],
                          top_k=top_k + resample['topk'])
