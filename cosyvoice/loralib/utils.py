...
def mark_only_lora_as_trainable(model: nn.Module, bias: str = 'none', debug=False) -> None:
    for n, p in model.named_parameters():
        if 'lora_' not in n:
            p.requires_grad = False
        if debug:
            print("检查梯度：", n, p.requires_grad)
    if bias == 'none':
        return
    elif bias == 'all':
        for n, p in model.named_parameters():
...
def getModelSize_lora(model, hps):
    ...
    for key, param_size in param_size_dict.items():
        print("lora节点{}大小为：{:.3f}MB".format(key, param_size))
        all_size += param_size
    print("lora模型总大小为：{:.3f}MB".format(all_size))
    if "enc_q" in param_size_dict:
        print("lora模型去除enc_q总大小为：{:.3f}MB".format(all_size-param_size_dict["enc_q"]))
    return

def adjust_r(name, hps, lora_r):
    lora_max = hps.lora_max #8
    lora_min = hps.lora_min #2
    # lora_lambda_flow = getattr(hps, "lora_lambda_flow", 1.0)
    # lora_lambda_dec = getattr(hps, "lora_lambda_dec", 1.0)
...
def replace_specific_layer_4lora(model, hps):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, torch.nn.Linear):
            ...
            ### NOTE ### NOTE ### NOTE ### NOTE ### NOTE 
            # lora_init_weights_local = "normal" if num_embeddings<=unique_dim or embedding_dim<=unique_dim else lora_init_weights
            ...
            if "noconvk" in hps.lora_init_weights:
                lora_init_weights_local = "normal" if module.kernel_size[0]>1 else lora_init_weights
            # print("{} type:{} init:{} r:{} in_c:{} out_c:{} ker:{} s:{} d:{} g:{}".format(name, type(module), lora_init_weights_local, lora_r_new, module.in_channels, module.out_channels, module.kernel_size[0], module.stride, module.dilation, module.groups))
            if getattr(module, "weight_g", None) is not None and "nownorm" in hps.lora_init_weights:
                lora_init_weights_local = "normal"
        ...
        ### module_list=(torch.nn.Linear, torch.nn.Embedding, torch.nn.Conv2d, torch.nn.Conv1d, torch.nn.ConvTranspose1d)
    return model

def handle_pissa_weight(model, hps, action="init"):
    lora_r = hps.lora_r #16
    lora_alpha = hps.lora_alpha #32
    lora_dropout = hps.lora_dropout #0.01
    lora_max = hps.lora_max #8
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, torch.nn.Linear):
            if action=="init":
                ...
                lora_init_weights_local = "normal" if out_features<=unique_dim or in_features<=unique_dim else lora_init_weights
                lora_r_new = adjust_r(name, hps, lora_r)
                # print("{} type:{} init:{} r:{} in_feat:{} out_feat:{}".format(name, type(module), lora_init_weights_local, lora_r_new, in_features, out_features))
                # print("{} {} r:{} in_feat:{} out_feat:{}".format(name, lora_init_weights_local.upper() if lora_init_weights_local!='normal' else "", lora_r_new, in_features, out_features))
                print("init {} {}".format(name, lora_init_weights_local.upper() if lora_init_weights_local!='normal' else ""))
                module.init_parameters()
            elif action=="unmerge":
                module.unmerge_parameters()
            elif action=="merge":
                module.merge_parameters()
        elif isinstance(module, torch.nn.Embedding):
            if action=="init":
                num_embeddings, embedding_dim = module.weight.shape
                device = module.weight.device
                ...
                lora_init_weights_local = "normal"
                lora_r_new = adjust_r(name, hps, lora_r)
                # print("{} {} r:{} num_emb:{} emb_dim:{}".format(name, lora_init_weights_local.upper() if lora_init_weights_local!='normal' else "", lora_r_new, num_embeddings, embedding_dim))
                print("init {} {}".format(name, lora_init_weights_local.upper() if lora_init_weights_local!='normal' else ""))
                module.init_parameters()
            elif action=="unmerge":
                module.unmerge_parameters()
            ...
            if action=="init":
                ...
                if getattr(module, "weight_g", None) is not None and "nownorm" in hps.lora_init_weights:
                    lora_init_weights_local = "normal"
                # print("{} {} r:{} in_c:{} out_c:{} trans ker:{}".format(name, lora_init_weights_local.upper() if lora_init_weights_local!='normal' else "", lora_r_new, module.in_channels, module.out_channels, module.kernel_size[0]))
                print("init {} {} {}".format(name, lora_init_weights_local.upper() if lora_init_weights_local!='normal' else "", device))
                if getattr(module, "weight_g", None) is not None and "rmwnorm" in lora_init_weights:
                    module = remove_weight_norm(module)
                module.init_parameters()
