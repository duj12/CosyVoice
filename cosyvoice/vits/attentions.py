...
from cosyvoice.speaker.modules import LayerNorm
...
class MultiHeadAttention(nn.Module):
    def __init__(self, channels, out_channels, n_heads, p_dropout=0.,
        ...
        if window_size is not None:
            ...
            self.emb_rel_k = nn.Parameter(
                torch.randn(n_heads_rel, window_size * 2 + 1,
    ...
    def attention(self, query, key, value, mask=None):
        ...
        query = query.view(b, self.n_heads, self.k_channels, t_t).transpose(2,
                                                                            3)
        key = key.view(b, self.n_heads, self.k_channels, t_s).transpose(2, 3)
        value = value.view(b, self.n_heads, self.k_channels, t_s).transpose(2,
                                                                            3)
        ...
        output = output.transpose(2, 3).contiguous().view(b, d,
                                                          t_t)  # [b, n_h, t_t, d_k] -> [b, d, t_t]
    ...
    def _absolute_position_to_relative_position(self, x):
        ...
        x_flat = F.pad(x_flat,
                       commons.convert_pad_shape([[0, 0], [0, 0], [length, 0]]))
